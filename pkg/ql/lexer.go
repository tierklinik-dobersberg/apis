...
func (s *Lexer) Scan() TokenInfo {
	...
	if ch == '>' {
		...
		if next == '=' {
			...
			return TokenInfo{GREATER_THAN_EQUAL, ">="}
		}
		s.unread()
	...
	case ch == ')':
		return TokenInfo{CLOSE_BRACKET, string(ch)}
	case ch == '%':
...
func isTokenComparator(tok Token) bool {
	return tok == GREATER_THAN || tok == GREATER_THAN_EQUAL || tok == LESS_THAN || tok == LESS_THAN_EQUAL || tok == EQUAL || tok == NOT_EQUAL || tok == PERCENT
}
