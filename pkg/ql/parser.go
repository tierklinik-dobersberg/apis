...
func (p *Parser) parseExpression() (Node, error) {
	...
	for tok != EOF {
		...
		case tok == WS:
			if exp.Field != nil && exp.Comparator != "" && (exp.Literal != "" || isValueEmpty) {
		...
		case exp.Comparator == "":
			if !isTokenComparator(tok) {
				return exp, fmt.Errorf("expected Comparator, got %v", tok)
			}
			exp.Comparator = lit
		...
		case exp.Literal == "":
			if tok != STRING {
				...
				if isValueEmpty {
					...
					return exp, fmt.Errorf("expected Value, got %v", tok)
				}
			}
			if lit == "" {
				isValueEmpty = true
			...
			if exp.Field.TypeResolver != nil {
				var err error

				exp.Value, err = exp.Field.TypeResolver.ResolveType(lit)
				if err != nil {
					exp.Value = nil
					return exp, fmt.Errorf("invalid field type: %v", lit)
				}
			}
	...
	if exp.Field != nil && exp.Comparator == "" {
		return exp, errors.New("found no comparator when expected")
	}
	return exp, nil
}
